...
# --- INICIALIZACIÓN DE VARIABLES (Solución al NameError de línea 22) ---
...
if not st.session_state.conectado:
    with st.expander("🔐 CLIC AQUÍ PARA INGRESAR Y MARCAR TU ESTADO", expanded=False):
        ...
        # Aquí puedes añadir un campo de clave si lo deseas validar en el futuro
        if st.button("INGRESAR AL SISTEMA"):
            if nom_acc and ape_acc:
                st.session_state.conectado = True
                # Se limpia el nombre (strip) para que coincida con "BRYAN " del Excel
    ...
    if b1.button("🟢 ESTOY LIBRE", use_container_width=True):
        with st.spinner("Actualizando..."):
            ...
            if "OK" in res: st.toast("✅ Estado: LIBRE en Excel")
    ...
    if b2.button("🔴 ESTOY OCUPADO", use_container_width=True):
        with st.spinner("Actualizando..."):
            ...
            if "OK" in res: st.toast("✅ Estado: OCUPADO en Excel")
            else: st.error(f"Error: {res}")
...
# --- 2. FORMULARIO DE REGISTRO (RESTAURADO COMPLETAMENTE) ---
    ...
    if not nombre or not email or not clave or not placa:
        ...
        with st.spinner("Guardando registro..."):
            ...
            if "REGISTRO_OK" in resultado:
                ...
                cuerpo = f"Socio: {nombre} {apellido}\nCédula: {cedula}\nPlaca: {placa}"
                link_email = f"mailto:{EMAIL_ADMIN}?subject={urllib.parse.quote(asunto)}&body={urllib.parse.quote(cuerpo)}"
                st.markdown(f'<a href="{link_email}" style="background-color:#0277BD; color:white; padding:15px; display:block; text-align:center; text-decoration:none; border-radius:10px; font-weight:bold;">📱 ENVIAR REQUISITOS</a>', unsafe_allow_html=True)
            else:
