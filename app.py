...
def registrar_viaje_en_sheets(datos):
    try:
        params = {
            ...
            "mapa": datos['mapa'],
            "id_viaje": datos['id_viaje'] # Aquí enviamos el ID único generado en Python
        }
...
if enviar:
    if not nombre_cli or not ref_cli:
        ...
        # --- SOLUCIÓN AL PROBLEMA DE ID ---
        # 1. Generamos el ID ÚNICO una sola vez aquí
        id_unico = f"TX-{random.randint(1000, 9999)}"
        
        with st.spinner("🔄 Procesando..."):
            chof, t_chof = obtener_chofer_libre()
            
            # 2. Registramos usando el ID_UNICO
            ...
            if chof:
                st.balloons()
                # 3. Mostramos el ID_UNICO en pantalla
                st.markdown(f'<div style="text-align:center;"><span class="id-badge">🆔 ID DE VIAJE: {id_unico}</span></div>', unsafe_allow_html=True)
                st.success(f"✅ ¡Unidad Encontrada! Conductor: **{chof}**")
                
                # 4. Enviamos el ID_UNICO por WhatsApp
