...
def obtener_chofer_libre():
    ...
    if not df.empty:
        # Filtramos solo los LIBRES y VALIDADOS
...
# --- MENÚ LATERAL PARA CAMBIAR DE MODO ---
modo = st.sidebar.selectbox("Menú Principal", ["Pedir Taxi 🚖", "Registro Conductores 📝"])

# ==========================================
# 🚖 MODO 1: INTERFAZ DE CLIENTE (ORIGINAL)
# ==========================================
if modo == "Pedir Taxi 🚖":
...
# ==========================================
# 📝 MODO 2: REGISTRO DE CONDUCTOR (NUEVO)
# ==========================================
elif modo == "Registro Conductores 📝":
    st.markdown("## 📝 Registro Oficial de Socio")
    st.info("Llena este formulario para activarte en la plataforma global.")
    ...
    with st.form("form_registro_oficial"):
        ...
        with col1:
            ...
            pais = st.selectbox("País *", ["Ecuador", "Colombia", "Perú", "México", "España", "USA"])
            direccion = st.text_input("Dirección *")
            clave = st.text_input("Crea tu Clave *", type="password")
            
        with col2:
            apellido = st.text_input("Apellido *")
            telefono = st.text_input("WhatsApp (con código) *")
            email = st.text_input("Email (Opcional)")
            placa = st.text_input("Placa Vehículo *")
            tipo_veh = st.selectbox("Tipo *", ["Taxi 🚖", "Camioneta 🛻", "Ejecutivo 🚔"])
            idioma = st.selectbox("Idioma *", ["Español", "English"])

        st.caption("Al registrarte aceptas los términos y el cobro de comisión por uso.")
        enviar_reg = st.form_submit_button("✅ REGISTRARME AHORA")
        
        if enviar_reg:
            if nombre and cedula and telefono and clave and placa:
                datos = {
                ...
                res = enviar_datos_a_sheets(datos)
                if "REGISTRO_EXITOSO" in res:
                    st.success("🎉 ¡Cuenta Creada! Ya estás activo.")
                    st.balloons()
                else:
