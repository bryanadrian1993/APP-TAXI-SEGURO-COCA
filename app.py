...
def obtener_chofer_libre():
    ...
    if not df.empty:
        # AQUÍ ESTÁ LA MAGIA: Filtramos solo los que están "LIBRE"
...
# ==========================================
# 🚖 INTERFAZ DE CLIENTE (ÚNICA PANTALLA)
# ==========================================
...
# --- MENSAJE INFORMATIVO DE LA BARRA LATERAL ---
st.sidebar.info("👋 **¿Eres Conductor?**\nHaz clic en 'Portal Conductores' arriba en el menú para ingresar.")
...
if enviar:
    if not nombre_cli or not ref_cli:
        ...
        with st.spinner("🔄 Buscando conductor libre..."):
            # 1. Buscamos chofer LIBRE en el Excel
            ...
            if chof:
                ...
                st.markdown(f'<a href="{link_wa}" class="wa-btn" target="_blank">📲 ENVIAR UBICACIÓN AL CONDUCTOR</a>', unsafe_allow_html=True)
            else:
                st.error("❌ No hay conductores 'LIBRES' en este momento.")
                st.info("Intenta de nuevo en unos minutos.")
